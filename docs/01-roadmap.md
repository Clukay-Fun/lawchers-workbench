...
> **明文存储决策（2026-06-24 调整）**：`entity.original` 存敏感原文，仅落本地 SQLite（gitignore、不出机器），用于刷新后重新定位脱敏；可逆还原仍以 legal-desens 的 `map.json` 为准。
...
见 README「数据模型」一节。要点：`entity.original` 存本地库（gitignore），可逆还原以 `map.json` 为准。
...
### 每阶段都适用的红线
- 遵守 AGENTS.md。敏感数据仅存本地（gitignore），绝不提交进 git。
